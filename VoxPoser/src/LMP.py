...
class LMP:
    ...
    def _cached_api_call(self, **kwargs):
        ...
        if "wizardcoder" in kwargs['model'].lower(): 
            # build up wizardcoder llm
            if LMP._llm is None:
                try:
                    base_model=f"WizardLM/{kwargs['model']}"
                    print(f"DEBUG: Loading wizardcoder model. {base_model}")
                    LMP._llm = LLM(model=base_model, tensor_parallel_size=1)
                    
                except Exception as e:
                    print(f'ERROR: Loading wizardcoder model failed.', e)
                    raise
            ...
            if kwargs in self._cache:
                ...
                # convert prompt to tensor float16
                #prompt = prompt.half().to(self._device)

                sampling_params = SamplingParams(
                    temperature=temperature, top_p=1, max_tokens=max_tokens, stop=stop_tokens
                )
                completions = LMP._llm.generate(prompt, sampling_params)
                ret = completions[0].outputs[0].text.strip()
                #print(f"////////////////DEBUG: LLM Generation Done. Code str\n:", ret) 
        ...
        if kwargs['model'] != 'gpt-3.5-turbo-instruct' and \
            ...
            if kwargs in self._cache:
                ...
                ret = ret.replace('```', '').replace('python', '').strip()
                self._cache[kwargs] = ret
                print("DEBUG MESSAGE: using gpt3.5 or gpt4.0. \n")
                return ret
            
        elif kwargs['model'] == 'wizardcoder':
            if kwargs in self._cache:
                print('(using cache)', end=' ')
                return self._cache[kwargs]
            else:
                ...
                self._cache[kwargs] = ret
                print("DEBUG MESSAGE: using wizardcoder. \n")
                return ret
            ...
            if kwargs in self._cache:
                ...
                ret = openai.Completion.create(**kwargs)['choices'][0]['text'].strip()
                self._cache[kwargs] = ret
                print("DEBUG MESSAGE: using gpt3.5-turbo-instruct. \n")
                return ret
