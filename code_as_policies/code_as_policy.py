...
class LMP:
    ...
    def __call__(self, query, context="", **kwargs):
        ...
        sampling_params = SamplingParams(
            temperature=self._cfg['temperature'], top_p=1, max_tokens=self._cfg["max_tokens"], stop=stop_tokens
        )
        completions = self._lmp_fgen.llm.generate(prompt, sampling_params)
        code_str = completions[0].outputs[0].text.strip()
        print(f"DEBUG: LLM Generation with temperature={self._cfg['temperature']} Done.")
...
class LMPFGen:
    ...
    def create_f_from_sig(
        ...
        print(f"DEBUG: newFunc Generation with temperature={self._cfg['temperature']} Done.")
